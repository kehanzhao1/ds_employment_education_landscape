...
#%%
skills_list = df['Responsibility'].tolist()
...
ds_terms = [
 "machine learning", "big data", "data management", "database management"
    ...
    , 'actionable insights', 'find trends', 'provide recommendations'
    ]
...
skills_list_rep= [[token if token !='ml' else 'machine_learning' for token in skills] for skills in skills_list_reduced]
...
wordcloud = WordCloud(width=800, height=400, background_color='white').generate_from_frequencies(Counter(skills_vis))
...
node_importance = Counter([token for skills in skills_list_network_cleaned 
                           for token in skills])
important_nodes = [node for node, count in node_importance.items() if count > 14]
filtered_skills_list = [[token for token in skills if token in important_nodes]for skills in skills_list_network_cleaned]
