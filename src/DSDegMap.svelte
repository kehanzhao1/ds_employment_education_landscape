<script>
    ...
    export let CCPath;
    export let variableToVisualize;
    ...
    onMount(async () => {
        ...
        bins.forEach(bin => {
            bin.numDS = d3.sum(bin, d => d.hasDS);
            bin.numDA = d3.sum(bin, d => d.hasDA);
            bin.cities = d3.map(bin, d => d.city);
        });

        const colorScale = d3.scaleQuantize()
            .domain([Math.floor(d3.min(bins, d => d[variableToVisualize])), Math.ceil(d3.max(bins, d => d[variableToVisualize]))])
        ...
        mapGroup.append("g")
            ...
            .attr("fill", d => colorScale(d[variableToVisualize]))
            .attr("stroke", "none")
            .attr("opacity", 0.8)
        ...
        function showTooltip(event, d) {
            const tableRows = d.map(college => `
                <tr>
                    ...
                    <td>${college.hasDS && college.hasCert ? '✅' : '❌'}</td>
                    <td>${college.hasDS && college.hasDeg ? '✅' : '❌'}</td>
                </tr>
            `).join('');
