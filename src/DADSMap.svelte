<script>
    ...
    onMount(async () => {
        ...
        bins.forEach(bin => {
            ...
            bin.numBoth = d3.sum(bin, d => d.hasDS || d.hasDA);
            bin.cities = d3.map(bin, d => d.city);
        });

        const colorScale = d3.scaleQuantize()
            .domain([Math.floor(d3.min(bins, d => d.numBoth)), Math.ceil(d3.max(bins, d => d.numBoth))])
        ...
        mapGroup.append("g")
            ...
            .attr("fill", d => colorScale(d.numBoth))
            .attr("stroke", "none")
            .attr("opacity", 0.8)
        ...
        function showTooltip(event, d) {
            ...
            d3.select(tooltip)
                ...
                .html(`
                    ...
                    <br><b>Number of Schools with DA or DS:</b> ${d.numBoth}
                    ${tableHTML}
                `);
        }
